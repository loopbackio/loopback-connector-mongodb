  ...
  if (self.db) {
    ...
    try {
      for(let i = 0; i<self.settings.stores.length; i++) {
        let collection = String(self.settings.stores[i]);
        if(typeof collection !== "string" || collection === "") continue;
        db[collection] = new nedb({...loadDatabase,  filename: path.join(self.settings.root, collection)});
  ...
  if (self.db) {
    ...
    async.eachSeries(
      ...
      function(modelName, modelCallback) {
        ...
        let opt = { filename: self.db[collectionName].filename };
