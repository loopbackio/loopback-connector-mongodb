...
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  // console.log(modelInfo.properties)
  // console.log(data)
  // SPIKE: Convert ObjectID to string for user consumption
  for (const propName in modelInfo.properties) {
    const propdef = modelInfo.properties[propName];
    if (isObjectIDprop(propdef) && data[propName]) {
      if (Array.isArray(data[propName])) {
        data[propName].map(item => item.toString());
      } else {
        data[propName] = data[propName].toString();
      }
    }
  }

  function isObjectIDprop(propdef) {
    if (propdef.mongodb && propdef.mongodb.dataType.match(ObjectIdTypeRegex)) {
      return true;
    } else if (propdef.type.name === 'ObjectID') {
      return true;
    }
    return false;
  }
...
// Property coercer for saved data
function propertyCoercer(data, propDefs) {
  // Convert ObjectID to string for user consumption
  ...
  function isObjectIDprop(propdef) {
    ...
    // SPIKE: We are not exposing ObjectID format anymore
...
function isStoredAsObjectID(propDef) {
  ...
  if (propDef.mongodb && ObjectIdTypeRegex.test(propDef.mongodb.dataType)) {
    return true;
  }
  return false;
}
...
// Tries to coerce a property into ObjectID after if type is declared as ObjectID
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  if (isStoredAsObjectID(propDef) || value instanceof mongodb.ObjectID) {
    return true;
  } else {
    return false;
  }
...
function coercePropertyValue(modelCtor, propValue, propDef, setValue) {
  ...
  if (propDef && propDef.mongodb && propDef.mongodb.dataType) {
    ...
    if (typeof dataType === 'string') {
      ...
      // Explicitly declaring the property as a ObjectID is the only criteria of setting it as an ObjectID now,
      // there will be no auto-conversion like how it used to work earlier
