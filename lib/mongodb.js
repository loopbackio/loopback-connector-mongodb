...
// SPIKE: strangely, this is called only if the model has an id property
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  for (const p in props) {
    ...
    if (prop && prop.type === Buffer) {
      ...
      // console.log(prop.type.definition.name, data[p])
  ...
  // SPIKE: required because of line 423
  if (modelCtor) {
...
MongoDB.prototype.coerceId = function(modelName, id, options) {
  ...
  if (idProp && typeof idProp.type === 'function') {
    ...
    // SPIKE: no more ObjectID coercion
    // const modelCtor = this._models[modelName];
  ...
  const xid = modelProps[idName];
  // console.log(xid);
  // console.log(modelCtor);
  return idValue;
};
...
MongoDB.prototype.buildWhere = function(modelName, where, options) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    // SPIKE: this will be true if user did not specify the id property
    } else if (propDef && propDef.id) {
      cond = ObjectID(cond);
    }
