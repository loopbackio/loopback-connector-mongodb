...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  this.execute(modelName, 'insertOne', data, {safe: true}, function(err, result) {
    ...
    try {
      ...
      oId = self.coercePropertyToString(modelName, idName, oId, true);
    } catch (err) {
      return callback(err);
    }
...
MongoDB.prototype.exists = function(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  id = self.coercePropertyToObjectId(modelName, idName, id, true);
...
MongoDB.prototype.coercePropertyToObjectId = function(modelName, propName, propValue, isId) {
  ...
  if (propDef && propDef.mongodb && propDef.mongodb.dataType && propDef.mongodb.dataType.match(/objectid/i)) {
