...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  for (var p in props) {
    ...
    } // check if prop contains EmbedsMany
    else if(prop && prop.type.constructor === Array &&
        prop.type[0].toString() === model.model.toString()){
      for(var k in settings.relations){
        if(settings.relations[k].property === p){
    ...
    } // check if prop contains EmbedsOne
    else if(prop && prop.type.toString() === model.model.toString()){
      for(var k in settings.relations){
        if(settings.relations[k].property === p){
