...
function buildNearFilter(query, node) {
  var coordinates = {};
  if (typeof node.near === 'string') {
    var s = node.near.split(',');

    coordinates.lng = Number(s[0]);
    coordinates.lat = Number(s[1]);
  } else {
    coordinates = node.near;
  }

  query.where[node.key] = {
    near: {
      $geometry: {
  ...
  var unit = node.unit || 'meters';

  props.forEach(function(p) {
    if (node[p]) {
      query.where[node.key]['near']['$' + p] = convertToMeters(node[p], unit);
    }
  });
};
