...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  for(var k in model.model.relations){
    if(model.model.relations[k].keyFrom){
      relations[model.model.relations[k].keyFrom] = model.model.relations[k];
    }
  }
  for (var p in props) {
    var prop = props[p];
