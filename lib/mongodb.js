...
MongoDB.prototype.update =
  MongoDB.prototype.updateAll = function updateAll(model, where, data, cb) {
    ...
    var acceptedOperators = ['$currentDate', '$inc', '$max', '$min', '$mul', '$rename', '$setOnInsert', '$set', '$unset'];

    var usedOperators = Object.keys(data).filter(function (item) {
      return acceptedOperators.indexOf(item) !== -1;
    });
    ...
    if (usedOperators.length === 0) {
      ...
      delete data[usedOperators[0]][idName];
      // Clean the data object and leave only the first found operator
      //(see https://github.com/strongloop/loopback-connector-mongodb/pull/70#issuecomment-68143689)
      parsedData[usedOperators[0]] = data[usedOperators[0]];
    }

    this.collection(model).update(where, parsedData, {multi: true, upsert: false},
      function (err, count) {
        if (self.debug) {
