...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if (typeof relations !== 'undefined'
    ...
    var aggregate = [];
    Object.keys(relations).map(function (name, index) {
      if (relations[name].type === 'hasMany'
        && typeof relations[name].through === 'undefined') {
        aggregate = aggregate.concat({
          $lookup: {
            from: relations[name].model,
            localField: idName,
            foreignField: relations[name].foreignKey,
            as: name
          }
        });
      } else if (relations[name].type === 'belongsTo') {
        aggregate = aggregate.concat({
          $lookup: {
            from: relations[name].model,
            localField: relations[name].foreignKey,
            foreignField: idName,
            as: name
          }
        });
