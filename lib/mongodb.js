...
/**
 * Coerce type of all object id properties to mongoDB's ObjectID
 *
 * @see https://github.com/strongloop/loopback-connector-mongodb/issues/363
 * @param {String} model The model name
 * @param {Object} data The model data
 * @returns {Object} The new model data
 */
MongoDB.prototype.convertAllObjectID = function(model, data) {
  var self = this;
  var props = self._models[model].properties;
  for (var p in props) {
    var prop = props[p];
    var value = data[p];
    if (value && self.isObjectIDProperty(model, prop, value)) {
      data[p] = ObjectID(value);
    }
  }
  return data;
};
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  data = self.convertAllObjectID(model, data);
