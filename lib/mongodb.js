...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  s.safe = s.safe !== false;
  s.w = s.w || 1;
  s.url = s.url ? encodeMongoDBURL(s.url) : generateMongoDBURL(s);
...
exports.encodeMongoDBURL = encodeMongoDBURL;

function encodeMongoDBURL(url) {
  let encodedUrl = url;
  const atIndex = encodedUrl.lastIndexOf('@');
  let protocol = 'mongodb://';
  const srvProtocol = 'mongodb+srv://';

  if (url.startsWith(srvProtocol)) protocol = srvProtocol;

  if (atIndex > protocol.length) {
    let userSegment = encodedUrl.substring(protocol.length, atIndex);
    const hostSegment = encodedUrl.substring(atIndex + 1);
    const atIndexUser = userSegment.indexOf(':');

    if (atIndexUser > -1) {
      let user = userSegment.substring(0, atIndexUser);
      let password = userSegment.substring(atIndexUser + 1);
      user = percentEncode(user);
      password = percentEncode(password);
      userSegment = user + ':' + password;
    }

    encodedUrl = protocol + userSegment + '@' + hostSegment;
  }

  return encodedUrl;
}

// Helper function for percent-encoding strings - https://tools.ietf.org/html/rfc3986#section-2.1
// This is required to support special characters in username and password
