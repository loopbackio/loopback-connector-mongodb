 ...
 * @param {Function} [cb] The callback function
 */
MongoDB.prototype.replaceAttributes = function replaceAttrs(model, id, data, options, cb) {
  var self = this;
  if (self.debug) {
    debug('replaceAttributes', model, id, data);
  }
  var oid = self.coerceId(model, id);
  this.replaceWithOptions(model, oid, data, {upsert: false}, cb);
};

/**
 * Update a model instance with id
 * @param {String} model The model name
 * @param {Object} where The search criteria which are key value pairs
 * @param {Object} data The property/value pairs to be updated or inserted if {upsert: true} is passed as options
 * @param {Object} options The options you wnat to pass for update, e.g, {upsert: true}
