 ...
 *
 * @param {String|Object} model The model name or the model class
...
MongoDB.prototype.fromDatabase = function (model, data) {
  ...
  if(settings){
    for(var k in settings.relations){
      if(settings.relations[k].property){
        relations[settings.relations[k].property] = settings.relations[k];
      }
    }
  }
  for (var p in props) {
    ...
    } // check if prop is a relation EmbedsMany
    else if(prop && relations[p] && relations[p].type === 'embedsMany'){
      for(var i = 0; i < data[p].length; i++){
        data[p][i] = self.fromDatabase({
          model: prop.type[0],
          properties: prop.type[0].definition.properties,
          settings: prop.type[0].definition.settings
        },data[p][i]);
      }
    } // check if prop is a relation EmbedsOne
    else if(prop && relations[p] && relations[p].type === 'embedsOne'){
      data[p] = self.fromDatabase({
        model: prop.type,
        properties: prop.type.definition.properties,
        settings: prop.type.definition.settings
      },data[p]);
    }
  }
  return data;
