...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  let queryObject = Object.assign({}, data);
  const oid = self.coercePropertyToObjectId(modelName, idName, idValue);
  queryObject._id = oid; // Use ObjectID for querying
  queryObject = self.toDatabase(modelName, data);

  this.execute(modelName, 'insertOne', queryObject, {safe: true}, function(err, result) {
    if (self.debug) {
    ...
    try {
      ...
      sId = self.coercePropertyToString(modelName, idName, oId);
    } catch (err) {
      return callback(err);
    }
...
MongoDB.prototype.exists = function(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  id = self.coercePropertyToObjectId(modelName, idName, id);
...
MongoDB.prototype.find = function find(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  const oid = self.coercePropertyToObjectId(modelName, idName, id);
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (propDef && propDef.mongodb && typeof propDef.mongodb.dataType === 'string') {
      if (Decimal128TypeRegex.test(propDef.mongodb.dataType)) {
        ...
        cond = self.coercePropertyToObjectId(modelName, propName, cond);
      }
    // For implicitly autogenerated ids
    } else if (propDef && propDef[idName]) {
      // When querying by id
      if (typeof cond === 'string') {
        cond = self.coercePropertyToObjectId(modelName, propName, cond);
      // When ids are included in filters
      // TODO: add other filters as encountered in tests
      } else if (cond.inq) {
        cond.inq = cond.inq.map(item => self.coercePropertyToObjectId(modelName, propName, item));
      }
    }
...
MongoDB.prototype.replaceById = function replace(modelName, id, data, options, cb) {
  ...
  const self = this;
  const idName = self.idName(modelName);
  const oid = this.coercePropertyToObjectId(modelName, idName, id);
...
MongoDB.prototype.coercePropertyToObjectId = function(modelName, propName, propValue) {
  ...
  if (propDef.id) {
    ...
    if (typeof propValue !== 'number') {
      ...
      // Id explicitly specified as ObjectID
      } else if (propDef.mongodb && propDef.mongodb.dataType && propDef.mongodb.dataType.match(/objectid/i)) {
        return ObjectID(propValue);
      }
    }
  } else if (propDef && propDef.mongodb && propDef.mongodb.dataType && propDef.mongodb.dataType.match(/objectid/i)) {
    propValue = ObjectID(propValue);
...
MongoDB.prototype.coercePropertyToString = function(modelName, propName, propValue) {
  if (propValue == null) return propValue;
  const self = this;
  const propDef = self.getPropertyDefinition(modelName, propName);
  if (propDef.id) {
  ...
  } else if (propDef && propDef.mongodb && propDef.mongodb.dataType && propDef.mongodb.dataType === 'string') {
    if (hasDataType('decimal128', propDef)) {
      if (Array.isArray(propValue)) {
        propValue = propValue.map(val => Decimal128.fromString(val));
      } else {
        propValue = Decimal128.fromString(propValue);
      }
    } else if (hasDataType('objectid', propDef)) {
      if (Array.isArray(propValue)) {
...
// TODO: use coercePropertyToString for coercion
function uncoercePropertyValue(modelCtor, propValue, propDef, setValue) {
  if (propDef) {
    let coercedValue;
    if (propDef.mongodb && propDef.mongodb.dataType) {
      const dataType = propDef.mongodb.dataType;
      if (typeof dataType === 'string') {
        if (hasDataType('decimal128', propDef)) {
          if (Array.isArray(propValue)) {
