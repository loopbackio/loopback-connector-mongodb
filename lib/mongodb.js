...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  if (idValue === null) {
    ...
    const oid = self.coercePropertyToObjectId(modelName, idName, idValue, true);
  ...
  this.execute(modelName, 'insertOne', data, {safe: true}, function(err, result) {
    ...
    // idValue comes as ObjectID from database
    idValue = result.ops[0]._id;
    try {
      const isId = true;
      // coerce idValue to string for user consumption, as result any code using
      // Mode.id will have a string value, therefore coerce before querying the database
      idValue = self.coercePropertyToString(modelName, idName, idValue, isId);
    } catch (err) {
      return callback(err);
    }
...
MongoDB.prototype.find = function find(modelName, id, options, callback) {
  ...
  const idName = self.idName(modelName);
  const oid = self.coercePropertyToObjectId(modelName, idName, id, true);
  ...
  Object.keys(where).forEach(function(k) {
    ...
    // For implicitly autogenerated ids
      ...
      // TODO: add other filters as encountered in tests
        ...
        cond.inq = cond.inq.map(item => ObjectID(item));
      }
    }
...
MongoDB.prototype.replaceById = function replace(modelName, id, data, options, cb) {
  ...
  const self = this;
  const idName = self.idName(modelName);
  const oid = this.coercePropertyToObjectId(modelName, idName, id, true);
...
MongoDB.prototype.coercePropertyToObjectId = function(modelName, propName, propValue, isId) {
  if (propValue == null) return propValue;
  const self = this;
  const propDef = self.getPropertyDefinition(modelName, propName);
  // Autogenerated id, so coerce to ObjectID
  if (isId && !propDef.mongodb) {
    return ObjectID(propValue);
  }
  if (propDef && propDef.mongodb && propDef.mongodb.dataType.match(/objectid/i)) {
...
MongoDB.prototype.coercePropertyToString = function(modelName, propName, propValue, isId) {
  if (propValue == null) return propValue;
  const self = this;
  const propDef = self.getPropertyDefinition(modelName, propName);
  // Autogenerated id, so coerce to string; leave numbers alone
  if (isId && !propDef.mongodb && typeof propValue !== 'number') {
...
// TODO: use coercePropertyToString for coercsion
