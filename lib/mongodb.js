...
MongoDB.prototype.create = function (model, data, callback) {
  ...
  var idType = self.idType(model);

  if (idValue === null) {
    delete data[idName]; // Allow MongoDB to generate the id
  } else {
    var oid = idType(idValue); // Is it an Object ID?
...
MongoDB.prototype.idType = function(model) {
  return this.getDataSource(model).idProperty(model).type;
};
