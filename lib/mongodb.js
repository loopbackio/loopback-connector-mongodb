...
function MongoDB(settings, dataSource) {
  ...
  this.defaultIdType = settings.defaultIdType || ObjectID;
...
MongoDB.prototype.create = function (model, data, callback) {
  ...
  if (idValue === null) {
    ...
    var oid = this.defaultIdType(idValue); // Is it an Object ID?
  ...
  this.collection(model).insert(data, {safe: true}, function (err, m) {
    ...
    } else if (idType === self.defaultIdType) {
      idValue = self.defaultIdType(idValue);
    }
...
MongoDB.prototype.save = function (model, data, callback) {
  ...
  var oid = this.defaultIdType(idValue);
  data._id = oid;
  idName !== '_id' && delete data[idName];
