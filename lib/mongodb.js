...
MongoDB.prototype.toDatabase = function(modelName, data) {
  ...
  for (const p in props) {
    ...
    // SPIKE: Ideally coercePropertyValue should handle this but it does not have enough info
      ...
      if (p === 'id' || prop.mongodb && prop.mongodb.dataType.match(/objectid/i)) {
        const objVal = data[p];
        try {
          data[p] = ObjectID(objVal);
        } catch (e) {
          console.log(e);
        }
      }
...
function coercePropertyValue(modelCtor, propValue, propDef, setValue) {
  ...
  // SPIKE: this will get simplified at lot once we remove ObjectID stuff
  // SPIKE: I am not touching them at the moment, lets focus on the minimal changes that's
  // SPIKE: required to make the PoC work
