...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  // create db property if it doesn't exist yet
  s.db = s.db || {};

  // define safe writeConcern default values
  // https://docs.mongodb.org/v3.0/reference/write-concern/
  var safeOptions = {
    w: 1,               // write to standalone server or at least primary in replSet
    j: true             // request acknowledgement from MongoDB that the write operation has been written to the journal
    // wtimeout: 1000   // timeout is only applicable for w > 1
  };

  // when safe option is set to "true", assign default values for "w" and "j" to db options
  if (s.safe === true) {
    Object.keys(safeOptions).forEach(function (option) {
      s.db[option] =  safeOptions[option];
    });
  }
