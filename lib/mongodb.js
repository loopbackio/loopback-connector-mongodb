...
/*!
 * Generate the mongodb URL from the config
 */
function generateMongoDBURL(config) {
  config.hostname = (config.hostname || config.host || '127.0.0.1');
  config.port = (config.port || 27017);
  config.database = (config.database || config.db || 'test');
  var options = null;
  if (config.options) {
    options = '?' + querystring.stringify(config.options);
  }
  var username = config.username || config.user;
  if (username && config.password) {
    return "mongodb://" + username + ":" + config.password + "@" + config.hostname + ":" + config.port + "/" + config.database + options;
  } else {
    return "mongodb://" + config.hostname + ":" + config.port + "/" + config.database + options;
  }
}
