...
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  for (const p in props) {
    ...
    } else if (isStoredAsObjectID(prop)) {
      if (typeof data[p] === 'string') {
        data[p] = ObjectID(data[p]);
      } else if (Array.isArray(data[p])) {
        const coercedArray = [];
        data[p].forEach(function(item, index) {
          const coerced = ObjectID(item);
          coercedArray.push(coerced);
        });
        data[p] = coercedArray;
      }
    } else if (
...
function isObjectIDProperty(modelCtor, propDef, value, options) {
  ...
  if (typeof value === 'string' && value.match(ObjectIdValueRegex)) {
    ...
    if (typeof firstItem === 'string'
      && firstItem.match(ObjectIdValueRegex)
      && isStoredAsObjectID(propDef)) {
      return true;
    }
