...
MongoDB.prototype.buildLookup = function (relation, include, model) {
  var self = this;
  var idName = self.idName(model);
  var lookup = [];
  if (relation.type === 'hasMany'
    && typeof relation.through === 'undefined') {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if (filter && filter.include) {
    ...
    if (Array.isArray(filter.include)) {
      filter.include.map(function (name, index) {
        aggregate = aggregate.concat(self.buildLookup(relations[name], name, model));
      });
    } else {
      aggregate = aggregate.concat(self.buildLookup(relations[filter.include], filter.include, model));
    }
    if (aggregate.length !== 0) {
      command = 'aggregate';
      if (Object.keys(query).length) {
        aggregate = aggregate.concat({ $match: query });
      }
      if (filter.limit) {
        aggregate = aggregate.concat({ $limit: filter.limit });
      }
      if (filter.skip) {
        aggregate = aggregate.concat({ $skip: filter.skip });
      }
      if (filter.order) {
        var order = {};
        ...
        aggregate = aggregate.concat({ $sort: order });
      }
      query = aggregate;
    }
  }
  if (fields) {
    this.execute(model, command, query, fields, processResponse);
  } else {
    this.execute(model, command, query, processResponse);
  }
  ...
  function processResponse(err, cursor) {
    ...
    if (command === 'aggregate') {
      if (self.debug) {
        debug('all', model, filter, err, cursor);
      }
      if (err) {
        return callback(err);
      }
      var objs = cursor.map(function (o) {
