...
function visitAllProperties(data, modelCtor, visitor) {
  ...
  for (const p of allProps) {
    ...
    if (def && def.type && isNestedModel(def.type)) {
      ...
      if (!def) {
        console.log(p, value, modelProps);
        throw new Error('rrr');
      }
...
MongoDB.prototype.coercePropertyToDecimal128 = function(modelName, propName, propValue) {
...
function coercePropertyToObjectId(propDef, propValue) {
  ...
  if (propDef.id) {
    ...
    if (typeof propValue !== 'number') {
      ...
      // Defined as String without mongodb definitions
      } else if (!propDef.mongodb && propDef.type === String) {
        return ObjectID(propValue);
...
function coercePropertyToString(propDef, propValue) {
  ...
  if (propDef.id) {
    ...
    if (typeof propValue !== 'number') {
      // Autogenerated id
      if (propDef.generated || propDef.type === String) {
